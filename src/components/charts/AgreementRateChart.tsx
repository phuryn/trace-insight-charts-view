...
const CustomTooltip = ({
  ...
  if (active && payload && payload.length) {
    return <div className="bg-white p-3 shadow-md border rounded-md">
...
const AgreementRateChart = ({
  data
}: AgreementRateChartProps) => {
  return <Card>
        ...
        <div className="h-[120px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data}>
              ...
              <YAxis domain={[0, 100]} tickCount={3} tick={{
              fontSize: 10
            }} axisLine={false} tickLine={false} tickFormatter={value => `${value}%`} />
              <Tooltip content={<CustomTooltip />} />
              <Line type="monotone" dataKey="agreementRate" stroke="#3b82f6" strokeWidth={2} dot={false} activeDot={{
              r: 4
            }} />
            </LineChart>
          </ResponsiveContainer>
